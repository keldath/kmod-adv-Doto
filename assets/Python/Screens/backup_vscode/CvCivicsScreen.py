...
class CvRegularCivicScreen:
	...
	def __init__(self):
		self.SCREEN_NAME = "RegularCivicScreen"
		self.CANCEL_NAME = "RegularCivicsCancel"
		self.RESET_NAME = "RegularReset"
		self.EXIT_NAME = "RegularCivicsExit"
		self.TITLE_NAME = "RegularCivicsTitleHeader"
		self.BUTTON_NAME = "RegularCivicsScreenButton"
		self.TEXT_NAME = "RegularCivicsScreenText"
		self.AREA_NAME = "RegularCivicsScreenArea"
		self.HELP_AREA_NAME = "RegularCivicsScreenHelpArea"
		self.HELP_IMAGE_NAME = "RegularCivicsScreenCivicOptionImage"
		self.DEBUG_DROPDOWN_ID =  "RegularCivicsDropdownWidget"
		self.BACKGROUND_ID = "RegularCivicsBackground"
		self.HELP_HEADER_NAME = "RegularCivicsScreenHeaderName"
		...
		self.HEADINGS_SPACING = -4 #5
		self.HEADINGS_BOTTOM = 330 #280
		self.HELP_TOP = 325 #350
		self.HELP_BOTTOM = 655#610
		self.TEXT_MARGIN = 14 #15
		self.BUTTON_SIZE = 24
		self.BIG_BUTTON_SIZE = 44 #64
		self.BOTTOM_LINE_TOP = 660#630
	...
	def getScreen(self):
		return CyGInterfaceScreen(self.SCREEN_NAME, CvScreenEnums.REGULAR_CIVICS_SCREEN)
...
#doto 115 goverment screen		
		self.HEADINGS_WIDTH = (self.W_SCREEN - self.HEADINGS_SPACING) / (gc.getNumCivicOptionInfos() - self.how_many_partent_child_civic_options()) - self.HEADINGS_SPACING
#doto 115 goverment screen
		# </advc.002b>
		...
		screen.setText(self.CANCEL_NAME, "Background", u"<font=4>" + "PREV_SELECT" + u"</font>", CvUtil.FONT_CENTER_JUSTIFY, self.X_CANCEL, self.Y_CANCEL, self.Z_TEXT, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, 1, 0)
		...
		# Header...
		screen.setText(self.TITLE_NAME, "Background", u"<font=4b>" + "CIVICS" + u"</font>", CvUtil.FONT_CENTER_JUSTIFY, self.X_SCREEN, self.Y_TITLE, self.Z_TEXT, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
	...
	def drawCivicOptionButtons(self, iCivicOption):
		...
		for j in xrange(gc.getNumCivicInfos()):
			...
			if (gc.getCivicInfo(j).getCivicOptionType() == iCivicOption):										
				...
				elif (activePlayer.canDoCivics(j, False)):
	...
	def drawAllButtons(self):				
		...
		for i in range(gc.getNumCivicOptionInfos()):
#doto 115 goverment screen			
			if not self.is_parent_child_goverment_civic(i):
#doto 115 goverment screen
				...
				szAreaID = self.AREA_NAME + str(reindex)
				screen = self.getScreen()
	...
	def CivicsButton(self, inputClass):
		...
		idx_factor = self.how_many_partent_child_civic_options()
		# civics position start after the goernment once, so gotta reset the index
	...
	def drawHelpText(self, iCivicOption, true_index = -1):
		...
		szPaneID = "CivicsHelpTextBackground" + str(true_index)
		screen = self.getScreen()
		...
		screen.setLabel(self.HELP_HEADER_NAME + str(true_index), "Background",  u"<font=3>" + gc.getCivicInfo(self.m_paeDisplayCivics[iCivicOption]).getDescription().upper() + u"</font>", CvUtil.FONT_CENTER_JUSTIFY, fX + self.HEADINGS_WIDTH/2, self.HELP_TOP + self.TEXT_MARGIN, 0, FontTypes.GAME_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1 )

		fY = self.HELP_TOP - self.BIG_BUTTON_SIZE
		szHelpImageID = self.HELP_IMAGE_NAME + str(true_index)
...
#doto 115 goverment screen			
			if not self.is_parent_child_goverment_civic(i):
				...
				szPaneID = "CivicsHelpTextBackground" + str(re_index)
				screen = self.getScreen()
	...
	def updateAnarchy(self):
		...
		i = 0
#doto 115 goverment screen			
# ill keep the effect of any civic change to cause revolution , both on govermenr and normal civics
	...
	def Revolution(self, inputClass):
		...
		if (inputClass.getNotifyCode() == NotifyCode.NOTIFY_CLICKED) :
			...
			for idx, civic in enumerate(self.allDeliveredCivics):
				if civic != self.m_paeDisplayCivics[idx]:
					self.allDeliveredCivics[idx] = self.m_paeDisplayCivics[idx]

			# doto 115 back to the main civic screen
			CvScreensInterface.showCivicsScreen(self.allDeliveredCivics, 'regularcivics')
