...
class CvCivicsScreen:
	...
	def drawHelpText(self, iCivicOption):
		...
		fX = self.INITIAL_SPACING + self.HELP_LEFT - self.PANEL_WIDTH + 3 * self.TEXT_MARGIN
		...
		if ((gc.getCivicInfo(iCivic).getUpkeep() != -1) and not activePlayer.isNoCivicUpkeep(iCivicOption)):
			...
			screen.setTextAt( szPaneIDUpkeep, "CivicList", u"<font=3>" + szHelpText + u"</font>", CvUtil.FONT_CENTER_JUSTIFY, fX + self.HEADINGS_WIDTH / 4 + 150, fY - self.TEXT_MARGIN , 0, FontTypes.GAME_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1 )
