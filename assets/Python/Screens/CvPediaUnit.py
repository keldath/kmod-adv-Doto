...
# MOD - START - Ranged Strike - Keldath addition for Doto
		...
		if (airCombat > 0 andregularCombat == 0):
			iStrength = airCombat
		else:
		...
		if (airRange > 0):
			...
			if (airCombat > 0):
				szName = self.top.getNextWidgetName()
				szRange = localText.getText("TXT_KEY_PEDIA_AIRCOMBAT", ( airCombat, ) )
				screen.appendListBoxStringNoUpdate(panelName, u"<font=4>" + airCombat.upper() + u"</font>", WidgetTypes.WIDGET_GENERAL, 0, 0, CvUtil.FONT_LEFT_JUSTIFY)
				
			szName = self.top.getNextWidgetName()
			szRange = localText.getText("TXT_KEY_PEDIA_RANGE", (airRange, ) )
