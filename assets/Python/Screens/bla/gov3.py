...
class CvGovermentScreen:
    ...
    def calculate_screen_metrics(self, screen):
        ...
        num_visible_civic_options = self.gc.getNumCivicOptionInfos() - self.get_parent_child_civic_options_count()
...
# ----------------------------------------------------------------------
    ...
    def draw_all_civic_options(self, screen): # Now requires 'screen' argument
        ...
        # Iterate over all Civic Options
        for option_id in range(self.gc.getNumCivicOptionInfos()):
            # Only draw columns for the Parent Civic Options
            if not self.is_parent_child_civic_option(option_id):
                continue
            
            # Skip if this option resulted in no civics (e.g., if the option is empty in XML)
            if option_id not in self.m_allCivicTreeData:
                continue
            ...
            # Get the custom tree data for this Civic Option
            option_tree = self.m_allCivicTreeData[option_id]
            
            for item in option_tree:
                
                # --- Draw PARENT Civic ---
                parent_civic_id = item['parent']['id']
                parent_civic_info = self.gc.getCivicInfo(parent_civic_id)
                
                can_do_civic = active_player.canDoCivics(parent_civic_id, False)
                is_displayed_civic = self.display_civics[option_id] == parent_civic_id
                ...
                if can_do_civic or is_displayed_civic:
                    ...
                    screen.setText(text_name, "", parent_civic_info.getDescription(), 
                    ...
                    # --- Draw CHILD Civics (Dependencies) ---
                    if item['children']:
                        for child_data in item['children']:
                            child_civic_id = child_data['id']
                            child_civic_info = self.gc.getCivicInfo(child_civic_id)
                            
                            y_pos += TEXT_MARGIN * 1.5 # Smaller step for child list
                            
                            # Child Icon (Small icon to indicate link)
                            child_icon_name = "ChildIcon%d_%d" % (child_civic_id, parent_civic_id)
                            # Image name is derived from civic, not self.HELP_IMAGE_NAME
                            screen.addDDSGFC(child_icon_name, 
                                            child_civic_info.getButton(), 
                                            int(x_pos + BUTTON_SIZE),
                                            int(y_pos + 2), 
                                            int(BUTTON_SIZE * 0.75), 
                                            int(BUTTON_SIZE * 0.75), 
                            ...
                            # Use smaller font and indentation for child civics
                            screen.setLabel(child_text_name, "Background", 
                                            u"<font=2>" + child_data['name'] + u"</font>",
                                            CvUtil.FONT_LEFT_JUSTIFY, 
                                            x_pos + BUTTON_SIZE * 1.8 + TEXT_MARGIN, y_pos, 0, 
    ...
    def handle_civics_button_click(self, input_class):
        ...
        # Only process clicks on actual Parent Civics (selectable checkboxes)
        if self.is_parent_child_civic_option(civic_info.getCivicOptionType()):
            # This is a child civic option, do not process as a selectable civic click
            return 0 
                ...
                if parent_civic_info.getCivicOptionType() == iCivicOption:
                    ...
                    for cChildIndex in xrange(childNum):
                        ...
                        child_data = {
                            ...
                            # Get the name of the Civic Option the child belongs to
                            'option_name': self.gc.getCivicOptionInfo(child_civic_info.getCivicOptionType()).getDescription(),
                            'id': child_civic_id,
                        }
                        child_civics_data.append(child_data)
                            
                    civic_list_for_option.append({
                        'parent': parent_data,
                        'children': child_civics_data
                    })
