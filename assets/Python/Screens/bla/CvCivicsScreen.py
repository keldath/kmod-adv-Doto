...
class CvCivicsScreen:
	...
	def __init__(self):
		...
		CvUtil.pyPrint("sagi init ( %s )" %(self.iActivePlayer))
	...
	def setActivePlayer(self, iPlayer):
		...
		CvUtil.pyPrint("sagi setActivePlayer fn( %s )" %(iPlayer))
...
#doto 115 goverment screen		
			...
			CvUtil.pyPrint(str(activePlayer.getCivics(i)))
#doto 115 goverment screen
		CvUtil.pyPrint("sagi setActivePlayer fn2( %s )" %(self.iActivePlayer))
	...
	def interfaceScreen (self):
		...
		if screen.isActive():
			CvUtil.pyPrint("sagi interfaceScreen isActive")
			return
...
#doto 115 goverment screen
		...
		CvUtil.pyPrint("sagi setActivePlayer aa( %s )" %(gc.getGame().getActivePlayer()))
	...
	def drawCivicOptionButtons(self, iCivicOption):
		...
		CvUtil.pyPrint("sagi drawCivicOptionButtons ( %d )" %(self.iActivePlayer))
		screen = self.getScreen()
...
#doto 115 goverment screen
	...
	def select(self, iCivic):
		activePlayer = gc.getPlayer(self.iActivePlayer)
		CvUtil.pyPrint("sagi select( %s )" %(activePlayer))
		# doto 115 goverment screen / civic parent child
		# cvgovermentscreen sends a call from canDoCivics which checks if to draw the civic buttons.
		# i added the true , cvgovermentscreen_ignore_hide so the display of civics would display despite the 
		# getGovermentConversionCounter being 0. that is because if the counter is 0 it would show the civics as attainable,
...
# ill keep the effect of any civic change to cause revolution , both on govermenr and normal civics
	...
	def Revolution(self, inputClass):
		...
		CvUtil.pyPrint("sagi Revolution iActivePlayer ( %s )" %(self.iActivePlayer))
		activePlayer = gc.getPlayer(self.iActivePlayer)
		CvUtil.pyPrint("sagi Revolution( %d )" %(self.iActivePlayer))
		#doto 115 test
		
		govermentScreen = CvMainCivicsScreen.CvMainCivicsScreen()
		def showGovermentScreen():
			if (-1 != CyGame().getActivePlayer()):
				govermentScreen.interfaceScreen(self.m_paeCurrentCivics)
		showGovermentScreen()
		return 
	...
	def handleInput(self, inputClass):
		CvUtil.pyPrint("sagi handleInput civic ( %s )" %(inputClass.getFunctionName()))
		CvUtil.pyPrint("sagi handleInput civic ( %s )" %(self.iActivePlayer))
		if (inputClass.getNotifyCode() == NotifyCode.NOTIFY_LISTBOX_ITEM_SELECTED):
			screen = self.getScreen()
			iIndex = screen.getSelectedPullDownID(self.DEBUG_DROPDOWN_ID)
			CvUtil.pyPrint("sagi getPullDownData( %s )" %(screen.getPullDownData(self.DEBUG_DROPDOWN_ID, iIndex)))
