...
# ----------------------------------------------------------------------
    ...
    def draw_all_civic_options(self, screen): # Now requires 'screen' argument
        ...
        # for option_id in range(self.gc.getNumCivicOptionInfos()):
            # Only draw columns for the Parent Civic Options (non-child options)        
        ...
        for civic_option_id, list_of_dicts in self.m_allCivicTreeData.items():
            for item in list_of_dicts:
                ...
                #     if self.get_parent_child_civic_options_count() == 0:
                #         continue
                #         self.is_parent_child_goverment_civic(option_id)
    ...
    def draw_all_help_text(self, screen):
        ...
        reindex = 0
        for option_id in range (self.gc.getNumCivicOptionInfos()):
            # Only draw help panels for the Parent Civic Options
            if self.is_parent_child_civic_option(option_id):
                continue
