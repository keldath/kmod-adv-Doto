...
class CvCivicsScreen:
	...
	def drawAllButtons(self):				
		...
		screen.setText("others", "",  u"<font=3>" + u"<color=255,255,0,255>%s</color>" % "Civics Table" + u"</font>", CvUtil.FONT_LEFT_JUSTIFY, 570, const_fY - 17, 0, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
			...
			if gc.getCivicOptionInfo(i).getParentCivicOption() < 1:
				...
				for j in range(gc.getNumCivicInfos()):
					if (gc.getCivicInfo(j).getCivicOptionType() == i):
						...
						screen.setText(self.getCivicsTextName(j), "",u"<font=2>" + gc.getCivicInfo(j).getDescription() + u"</font>", CvUtil.FONT_LEFT_JUSTIFY, fx_text_width, fY, 0, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
						civic_counter += 1
