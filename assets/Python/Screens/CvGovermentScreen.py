...
# ----------------------------------------------------------------------
    ...
    def interfaceScreen (self, civics = None):
        ...
        screen.addScrollPanel("ReligionList", u"", 0, 200, self.screen_width, 155, PanelStyles.PANEL_STYLE_EXTERNAL )
                ...
                for item in list_of_dicts:
                    ...
                    if item['children']:
                        ...
                        for child_option_group in item['children']:
                            ...
                            child_option_id = child_option_group['option_id']
                            
                            option_header_name = "ChildOptionHeader%d_%d" % (child_option_id, parent_civic_id)
                            ...
                            for child_civic_data in child_option_group['civics']:
                                ...
                                is_displayed_civic = self.display_civics[child_option_id] == child_civic_id
                                ...
                                screen.setState(button_name_child, self.current_civics[child_option_id] == child_civic_id)  
    ...
    #                 civic_to_draw_text = self.get_group_civics_to_update(option_id, True)
    #                 # civic_id = self.display_civics[option_id]
    #                 y_pos = HELP_TOP
    #                 height = (HELP_BOTTOM - HELP_TOP)/ len(civic_to_draw_text)

    #                 for civic_id in civic_to_draw_text:
    #                     # Background panel for help text
    #                     pane_id = "CivicsHelpTextBackground" + str(option_id) + str(civic_id) + str(reindex)
    #                     screen.addPanel(pane_id, "", "", True, True, 
    #                                     x_pos, y_pos, self.headings_width, height, 
    #                                     PanelStyles.PANEL_STYLE_MAIN)
                        
    #                     # TODO
    #                     self.draw_selected_help_text(screen, civic_to_draw_text, x_pos, y_pos, height)
    ...
    #     # Upkeep string
    #     if ((self.gc.getCivicInfo(iCivic).getUpkeep() != -1) and not activePlayer.isNoCivicUpkeep(iCivicOption)):
    #         szUpkeepText = self.gc.getUpkeepInfo(gc.getCivicInfo(iCivic).getUpkeep()).getDescription()
    #     else:
    ...
    #     civic_option_text = self.gc.getCivicOptionInfo(iCivicOption).getDescription().upper() + "--> "
    #     civic_text = self.gc.getCivicInfo(self.m_paeDisplayCivics[iCivicOption]).getDescription().upper()  + " --" +  szUpkeepText
    ...
    def delete_help_text(self, screen, civics_to_update):

        selected_parent = None
        for civic in civics_to_update:
            civic_info = self.gc.getCivicInfo(civic)
            civic_option = civic_info.getCivicOptionType()
            if self.gc.getCivicOptionInfo(civic_option).getParentCivicOption() == 2:
                selected_parent = civic
                panel_name = self.HELP_AREA_NAME + str(selected_parent) + str(civic_option)
                screen.deleteWidget(panel_name)
                continue
        try:        
            for civic in civics_to_update:
                civic_info = self.gc.getCivicInfo(civic)
                civic_option = civic_info.getCivicOptionType()
                if self.gc.getCivicOptionInfo(civic_option).getParentCivicOption() == 1:
                    panel_name = self.HELP_AREA_NAME + str(selected_parent) + str(civic_option)
                    screen.deleteWidget(panel_name)
                    continue
        except Exception, e:
            CvUtil.pyPrint("missing screen ( %s )" %(e))
            raise e
           
    def delete_all_help_text(self, screen):

        parent = None
        numcivics = xrange(self.gc.getNumCivicOptionInfos())
        numcivicstypes = xrange(self.gc.getNumCivicOptionInfos())
        parents = []
        for civic_option in numcivics:
            if self.gc.getCivicOptionInfo(civic_option).getParentCivicOption() == 2:
                for civic_id in numcivicstypes:
                    civic_info = self.gc.getCivicInfo(civic_id)
                    if civic_info.getCivicOptionType() == civic_option:
                        panel_name_child = self.HELP_AREA_NAME + str(civic_id) + str(civic_option)
                        screen.deleteWidget(panel_name_child)
                        parents.append(civic_id)
                break

        CvUtil.pyPrint("sagi wparents init ( %s )" %(parents))  
        for civic_option1 in numcivics:     
            if self.gc.getCivicOptionInfo(civic_option1).getParentCivicOption() == 1:
                CvUtil.pyPrint("sagi civic_option1 init ( %s )" %(civic_option1))  
                for parent in parents:
                    panel_name_child = self.HELP_AREA_NAME + str(parent) + str(civic_option1)
                    screen.deleteWidget(panel_name_child)

    def draw_help_text(self, screen, civics_to_update = None):

        # Recalculate reindex based on actual option_id relative to parent options
        selected_parent = 0
        for option_id in range (self.gc.getNumCivicOptionInfos()):
            if self.gc.getCivicOptionInfo(option_id).getParentCivicOption() == 2:
                selected_parent = self.display_civics[option_id]

        count_parent_and_child_options = 0
        for i in range (self.gc.getNumCivicOptionInfos()):
            if self.gc.getCivicOptionInfo(i).getParentCivicOption() > 0:
                count_parent_and_child_options += 1

        y_pos = HELP_TOP
        height = ((HELP_BOTTOM - HELP_TOP)/ count_parent_and_child_options) + int(TEXT_FACTOR / 2 )     

        which_civic_set = self.display_civics
        if civics_to_update:
            which_civic_set = {}
            for civic in civics_to_update:
                civic_to_update_info = self.gc.getCivicInfo(civic)
                civic_to_update_option = civic_to_update_info.getCivicOptionType()
                which_civic_set[civic_to_update_option] = civic
                
              
        for option_id in range (self.gc.getNumCivicOptionInfos()):
            if self.gc.getCivicOptionInfo(option_id).getParentCivicOption() > 0:
                civic_id = which_civic_set[option_id]
                civic_info = self.gc.getCivicInfo(civic_id)
                civic_option = civic_info.getCivicOptionType()
                active_player = self.gc.getPlayer(self.active_player_id)

                # clean previous drawn test for the same panel
                panel_name = self.HELP_AREA_NAME + str(selected_parent) + str(civic_option)
                # self.original_civics
                screen.deleteWidget(panel_name)
                # 1. Build Help Text String
                ...
                # 2. Calculate Position
                x_pos = HEADINGS_SPACING + (self.headings_width + HEADINGS_SPACING) * selected_parent 

                # 3. Draw Header - Use self.HELP_HEADER_NAME
                # screen.setLabel(self.HELP_HEADER_NAME + str(selected_parent) + str(civic_option), "Background", 
                #                 u"<font=3>" + civic_info.getDescription().upper() + u"</font>", 
                #                 CvUtil.FONT_CENTER_JUSTIFY, 
                #                 int(x_pos), int(y_pos), int(self.headings_width), int(height), 0, 
                #                 FontTypes.GAME_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
                
                # screen.setLabel(self.HELP_HEADER_NAME + str(selected_parent) + str(civic_option), "Background", "Background", 
                #                              u"<font=3>" + civic_info.getDescription().upper() + u"</font>",
                #                              CvUtil.FONT_CENTER_JUSTIFY, 
                #                              int(x_pos), int(y_pos), 0, 
                #                              FontTypes.GAME_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
                # 4. Draw Multiline Text Area - Use self.HELP_AREA_NAME

                screen.addMultilineText(panel_name, help_text, 
                                        int(x_pos), int(y_pos), 
                                        int(self.headings_width), int(height), 
                                        WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
                y_pos += height - TEXT_SPACING   
    ...
    def draw_all_help_text(self, screen):
        ...
        how_many_parant_civics = 0
        for i in range(self.gc.getNumCivicInfos()):
            parent_civic_info = self.gc.getCivicInfo(i)	
            if self.gc.getCivicOptionInfo(parent_civic_info.getCivicOptionType()).getParentCivicOption() == 2:
                how_many_parant_civics += 1

        height = ((HELP_BOTTOM - HELP_TOP)/ count_parent_and_child_options) + TEXT_FACTOR

        for i in xrange(how_many_parant_civics):
            y_pos = HELP_TOP
            for j in xrange(count_parent_and_child_options):
                x_pos = HEADINGS_SPACING + (self.headings_width + HEADINGS_SPACING) * i
                pane_id = "CivicsHelpTextBackground" + str(i) + str(j)
                screen.addPanel(pane_id, "", "", True, True, 
                                x_pos, y_pos, self.headings_width, height, 
                                PanelStyles.PANEL_STYLE_MAIN)
                y_pos += height - TEXT_SPACING
        
        self.draw_help_text(screen)
    ...
    def get_group_civics_to_update(self, civic_id, should_highlight):
        ...
        if not should_highlight:
            """
            this entire section is for just one simple bug
            that changes child civics if changes are made while the parent of the changes child is
            already selected. without it, it moved the other child civic option to flip to the alternative civic
            in that other civic child option
            """
            delivered_civic = civic_id
            aa = []
            selected_parent_civic = None
            is_change_other_child = False
            which_civic_type_tokeep_from_displayed = None
            for civic_option_id, list_of_dicts in self.m_allCivicTreeData.items():
                for item in list_of_dicts:
                    # Check if the 'parent' key exists and contains an 'id'
                    if 'parent' in item and 'id' in item['parent']:
                        parent_civic = item['parent']['id']
                        if self.current_civics[0] == parent_civic:
                            selected_parent_civic = parent_civic
                            aa.append(selected_parent_civic)
                            CvUtil.pyPrint("sagi selected_parent_civic ( %s )" %(selected_parent_civic))
                            for item2 in list_of_dicts:
                                if 'parent' in item2 and 'id' in item2['parent']:
                                    parent_civic2 = item2['parent']['id']
                                    CvUtil.pyPrint("sagi parent_civic2 ( %s )" %(parent_civic2))    
                                if  parent_civic2 == selected_parent_civic:
                                    CvUtil.pyPrint("match found ( %s )" %(parent_civic2))
                                    for child_items in item2['children']:
                                        CvUtil.pyPrint("sagi child_items ( %s )" %(child_items))  
                                        for child in child_items["civics"]:
                                            aa.append(delivered_civic)
                                            CvUtil.pyPrint("sagi child_items ( %s )" %(child["name"]))
                                            CvUtil.pyPrint("sagi delivered_civic ( %s )" %(delivered_civic))    
                                            if child["id"] == delivered_civic:
                                                is_change_other_child = True
                                                which_civic_type_tokeep_from_displayed = child_items["option_id"]
                                                break

            
            if is_change_other_child:
                for civic_option_id, list_of_dicts in self.m_allCivicTreeData.items():
                    for item in list_of_dicts:
                        for child_items in item['children']:
                            if which_civic_type_tokeep_from_displayed:
                                if child_items["option_id"] != which_civic_type_tokeep_from_displayed:
                                    CvUtil.pyPrint("sagi display_civics ( %s )" %(child_items["option_id"]))  
                                    aa.append(self.current_civics[child_items["option_id"]])
                                    CvUtil.pyPrint("sagi is_change_other_child ( %s )" %(list(set(aa))))  
                                    return list(set(aa))

            CvUtil.pyPrint("passed it ( %s )" %(list(set(aa)))) 
        ...
        group = list(set(which_civics_to_highlight))
        return group
    ...
    def handle_civics_button_click(self, input_class):
        ...
        if notify_code == NotifyCode.NOTIFY_CLICKED:
            if input_class.getFlags() & MouseFlags.MOUSE_RBUTTONUP:
                ...
                civics_to_update = self.get_group_civics_to_update(civic_id, False)
                CvUtil.pyPrint("sagi NOTIFY_CLICKED ( %s )" %(civics_to_update))
                if civics_to_update:
                    self.delete_all_help_text(screen) # keep only the clicked
                    self.draw_help_text(screen, civics_to_update)
                    self.update_anarchy_and_buttons(screen)
            ...
            if civics_to_update:
                for civic_id in civics_to_update:
                    CvUtil.pyPrint("sagi hoverdamn ( %s )" %(civics_to_update))
                    if not self.highlight_civic(civic_id):
                        CvUtil.pyPrint("sagi hoverdamn2 ( %s )" %(self.highlight_civic(civic_id)))
                        # return
                self.draw_help_text(screen, civics_to_update)
                self.update_anarchy_and_buttons(screen)
            # if civics_to_update:
            ...
            if civics_to_update:
                for civic_id in civics_to_update:
                    if not self.unhighlight_civic(civic_id):
                        return
                self.delete_help_text(screen, civics_to_update)
                self.update_anarchy_and_buttons(screen)
    ...
    def reset_to_original(self, inputClass):
        if (inputClass.getNotifyCode() == NotifyCode.NOTIFY_CLICKED):
            ...
            self.set_active_player(self.active_player_id, self.current_civics)
            screen = CyGInterfaceScreen(self.SCREEN_NAME, CvScreenEnums.GOVERMENT_SCREEN)
            self.draw_contents(screen)
            self.update_anarchy_and_buttons(screen)
