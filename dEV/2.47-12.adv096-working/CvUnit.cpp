...
int CvUnit::LFBgetDefenderRank(const CvUnit* pAttacker) const
{
	...
	//vincentz ranged strike - priototize attackers with air limits.
	if (validCombatLimits(this, pAttacker)) {
...
*/
	...
	//////
	//vincentz ranged strike - priototize attackers with air limits.
	int activecombatlimit = 100;

	if (pAttacker->airBaseCombatStr() > 0 && pAttacker->airCombatLimit() > 0
		&& pAttacker->airCombatLimit() < 100) 
	{
		activecombatlimit = pAttacker->airCombatLimit();
	}
	else if (pAttacker->combatLimit() > 0 && pAttacker->combatLimit() < 100
			 && pAttacker->airCombatLimit() == 0) 
	{
		activecombatlimit = pAttacker->combatLimit();
	}
	

	//iDefenderHitLimit = maxHitPoints() - pAttacker->combatLimit();
	iDefenderHitLimit = maxHitPoints() - activecombatlimit;
