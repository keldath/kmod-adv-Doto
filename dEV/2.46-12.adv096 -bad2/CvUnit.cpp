...
bool CvUnit::validCombatLimits(CvUnit const* pDefender, CvUnit const* pAttacker) const
{
	...
	if (aDomain != DOMAIN_AIR && dDamage < aAirLimit)
	{
		return true;
	}	 
	//NONE AIR UNITS WITH NO AIR ATTACK
	if (aDomain != DOMAIN_AIR && (aAirLimit == 0  || aAirLimit == 100))
	{
		return true;
	}	
	//NONE AIR UNITS WITH COMBAT LIMIT AND NO AIR ATTACK VALUE
	if (aDomain != DOMAIN_AIR && dDamage < aCombatLimit && aBaseCombatStr == 0)	
	{
		return true;
	}
	//NONE AIR UNITS WITH NO COMBAT LIMIT AND NO AIR ATTACK VALUE
	if (aDomain != DOMAIN_AIR && (aCombatLimit == 0 || aCombatLimit == 100) && aBaseCombatStr == 0)	
	{
		return true;
	}	
