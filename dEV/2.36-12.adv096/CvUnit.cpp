...
// Vincentz Rangestrike cannot move
	if (bAttack)
	{
		//keldath - newer code from vip master git
...
int CvUnit::rangeCombatDamage(const CvUnit* pDefender) const
{
	...
	//Vincentz Random Damage Start - keldath - original line was without int	
...
/* kelfth - this is the ranged code from the stand alone, the above is newer
//	iDamage = std::max(1, ((GC.getDefineINT("RANGE_COMBAT_DAMAGE") * (iOurStrength + iStrengthFactor)) / (iTheirStrength + iStrengthFactor)) * );
	iDamage = std::max(1, (((GC.getGame().getSorenRandNum(100, "Random")) * (GC.getDefineINT("RANGE_COMBAT_DAMAGE") * (iOurStrength + iStrengthFactor)) / (iTheirStrength + iStrengthFactor)) / 100));
// Vincentz Rangestrik Random Damage start
//	iDamage *= ((GC.getGame().getSorenRandNum(100, "Random")) / 100);
//	iDamage /= 100;
//	iDamage += 1;
// Vincentz Rangestrike Random Damage end
*/
	return iDamage;
}
...
bool CvUnit::canRangeStrike() const
{
	...
	//Vincentz Rangestrike keldath - in the stand alone this was un marked
	//if (airRange() <= 0)
	//{
	//	return false;
	//}
...
//RANGED STRIKE VINCENTZ
	...
	{	//Keldath vincent set a move multiplier - i dont think i need this multiplier
	...
	if (pCity != NULL && (pCity->isBombardable(this)))
	{
		...
		if (((GC.getGame().getSorenRandNum(GC.getDefineINT("RANGESTRIKE_DICE"), "Random")) + airBaseCombatStr() * GC.getDefineINT("RANGESTRIKE_HIT_MODIFIER") * currHitPoints() / maxHitPoints()) < ((GC.getGame().getSorenRandNum(GC.getDefineINT("RANGESTRIKE_DICE"), "Random")) + pDefender->baseCombatStr() * pDefender->currHitPoints() / pDefender->maxHitPoints()))
		{
			...
			//gDLL->getInterfaceIFace()->addMessage(pDefender->getOwner(), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_COMBAT", MESSAGE_TYPE_INFO, getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_GREEN"), this->getX(), this->getY(), true, true);
			...
			gDLL->getInterfaceIFace()->addMessage(getOwner(), true, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_COMBAT", MESSAGE_TYPE_INFO, pDefender->getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_RED"), pPlot->getX(), pPlot->getY());
		}
		else
		{
			...
			//red icon over attacking unit
			//gDLL->getInterfaceIFace()->addMessage(pDefender->getOwner(), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_COMBAT", MESSAGE_TYPE_INFO, getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_RED"), this->getX(), this->getY(), true, true);
...
/* keldath - code from original stand alone ranged attack
	iDamage = rangeCombatDamage(pDefender);
	iUnitDamage = std::max(pDefender->getDamage(), std::min((pDefender->getDamage() + iDamage), airCombatLimit()));
//Vincentz Rangestrike check
	if ((GC.getGame().getSorenRandNum(100, "Random")) > 50 + airBaseCombatStr() + pPlot->getNumUnits() - pDefender->baseCombatStr())	
	{
		...
		gDLL->getInterfaceIFace()->addMessage(pDefender->getOwner(), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_COMBAT", MESSAGE_TYPE_INFO, getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_GREEN"), this->getX(), this->getY(), true, true);
		//white icon over defending unit
		gDLL->getInterfaceIFace()->addMessage(pDefender->getOwner(), false, 0, L"", "AS2D_COMBAT", MESSAGE_TYPE_DISPLAY_ONLY, pDefender->getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_WHITE"), pDefender->getX(), pDefender->getY_(), true, true);
...
/*Vincentz Rangestrike Strikeback - keldtah - remove due to the warnig above of oos - i dont care if a unit is vanished after its dead without animation
	...
	if (GC.getDefineINT("RANGESTRIKE_RETURN_FIRE") == 1)
	{
		...
		if (pDefender != NULL)
		{
			...
			if (((GC.getGame().getSorenRandNum(GC.getDefineINT("RANGESTRIKE_DICE"), "Random")) + pDefender->airBaseCombatStr()) * GC.getDefineINT("RANGESTRIKE_HIT_MODIFIER") > ((GC.getGame().getSorenRandNum(GC.getDefineINT("RANGESTRIKE_DICE"), "Random")) + this->baseCombatStr()))
			{
				...
				gDLL->getInterfaceIFace()->addMessage(getOwner(), true, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_COMBAT", MESSAGE_TYPE_INFO, this->getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_RED"), pPlot->getX(), pPlot->getY());
...
/*keldath- from ranged stand alone - the above is newer
//Vincentz Rangestrike Strikeback
	if (pDefender->canRangeStrikeAt(plot(), plot()->getX(), plot()->getY()))
	{
		iDamage = rangeCombatDamage(this);
		iUnitDamage = std::max(this->getDamage(), std::min((this->getDamage() + iDamage), pDefender->airCombatLimit()));

		if ((GC.getGame().getSorenRandNum(100, "Random")) > 50 + airBaseCombatStr() - pDefender->airBaseCombatStr() * 2)	
		{
			...
			this->setDamage(iUnitDamage, getOwner(), false);
			collateralCombat(plot(), this);
		}
		else
		{
