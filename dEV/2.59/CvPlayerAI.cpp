...
// COUNT BAD TILES
	// <advc.040>//in 095 there was a custom code for doto similar to this
	bool bFirstColony =
...
// NON-YIELD BONUS VALUE (ResourceValue)
		...
		if (eBonus != NO_BONUS) // advc.040: Area checks moved
		{
			if(!isBarbarian() && // advc.303: Barbarians don't care about bonus trade
				...
				//ADVC CHANGE FROM 40 RO 56.5 - F1RPO - KELDATH
...
// CITIES-PER-AREA CHECKS
	...
	//code exists in custom 095 doto more changes in the below part - i guess f1 fixed more in 096
...
//105 keldath from advc 097 - AI tech evaluation: Higher value for early-game resource yields
					{
						iYieldValue -= 100; // compare to a hypothetical low-value improvement
...
bool CvPlayerAI::AI_considerOffer(PlayerTypes ePlayer,
	...
	if (iChange < 0) {
		...
		// <advc.133>
		//f1rpo Revised AI evaluation of ongoing resource trades - keldath add
...
//f1rpo master merge - Bugfix: Sign error in AI resource trade evaluation
