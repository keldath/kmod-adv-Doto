	...
	bool bObsolete) const // advc.004c
{
	...
	for (int iPass = 0; iPass < 2; iPass++)
	{
		...
		if (iPass > 0)
		{
			...
			//doto civic dependents are not to be configed in the xml. just a safe guard here for the future
			//FAssertMsg(GC.getInfo(eCivicOption).getParentCivicOption() != 0, "buildings must not give civic option of civic dependency type -> xml config");
