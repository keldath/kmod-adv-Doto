...
//rangedattack-keldath Randomized Airbomb
	...
	else if (!GC.getGame().isOption(GAMEOPTION_RAND_BOMB_DMG))
	{
		pCity->changeDefenseModifier(-airBombCurrRate());
	}
	else 
...
//keldath-rangedattack-this is the original function
CvCity* CvUnit::bombardTargetOriginal(CvPlot const& kPlot) const
{
...
/*	get city plot using a ranged value.
	original code use direction and range =>1
...
*/
	if (!GC.getGame().isOption(GAMEOPTION_RANGED_ATTACK))
	{
		return bombardTargetOriginal(kPlot);
	}
...
//sort of a silly cheeck , should be either at some bool check if we want to test this, or have a canrangedbombard()
	if (&kPlot == this->plot() /*&& !GC.getGame().isOption(GAMEOPTION_RANGED_ATTACK)*/)
	{
...
//rangedstrike-keldath-editted
bool CvUnit::bombard(CvPlot const* pPlot)
{
//kplot != null =>city to bombard is known, no need to look for one.
	if (pPlot == NULL)
	{
		//if no plot was sent it means it uses normal rules
		//i didnt check for gameoptions incase ill have normal bombard units with ranged option on
...
//find a target in case we didnt get one.
	...
	if (pPlot == NULL) 
	{
		 ...
		 {
			pBombardCity = bombardTargetOriginal(getPlot()); 
		 }
	}
	else 
	{
...
//rangedattack-keldath random Bombard result end
	...
	//dont push bombard mission. the ranged attack function will continue
		if((/*canRangeStrikeK()*/rangedStrike() > 0) /* && getDomainType() != DOMAIN_AIR*/) 
		{	
			return true;
		}
		else
