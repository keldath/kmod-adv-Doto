...
//rangedstrike-keldath
		if (collateralDamage() > 0 || rangedStrike() > 0)
		{
			return 15; // was 17
		}
		...
		{	// K-Mod. I've rearranged and rewritten most of this block - removing the bbai code.
			...
			if (bTargetTooStrong)
			{
				...
				else
				{
					...
					{	
						getGroup()->pushMission(MISSION_RANGE_ATTACK, pTargetCity->getX(), pTargetCity->getY(), 0);	
						return;
					}
...
bool CvUnitAI::AI_bombardCity()
{
	...
	//so, if it couldnt fund a city target, and we do have ranged attack,
	//lets stop and not push a bomvard mission. since ranged covers it.
...
//rangedattack-keldath - lets try to range bombard first.
		...
		else if (canBombard(getPlot()) && rangedStrike() == 0)
		{
...
//rangedattack-keldath-qa1-maybe?	
			...
			else if (bBombardUnitCanBombardNow && (kGroup.canBombard(*pBestBombardPlot)) && rangedStrike() == 0)
			{
...
bool CvUnitAI::AI_followBombard()
{
	...
	{	
 	...
 	else if(canBombard(*plot()) && rangedStrike() == 0) 
	{
		getGroup()->pushMission(MISSION_BOMBARD);
		return true;
	}
		...
		if (AI_mayAttack(p.getTeam(), p))
		{
				...
				getGroup()->canMoveOrAttackInto(p, true, true))
			{
				...
				if (iPowerThreshold < 0)
				{
					...
					//rangedstrike-keldath
					if (getGroup()->getHeadUnit()->bombardRate() > 0 || getGroup()->getHeadUnit()->rangedStrike() > 0)
					{
