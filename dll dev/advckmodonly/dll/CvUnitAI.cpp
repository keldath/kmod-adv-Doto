...
void CvUnitAI::AI_attackCityMove()
{
	...
	if (pTargetCity != NULL)
	{
		...
		// K-Mod note: AI_compareStacks will try to use the AI memory if it can't see.
		{
			...
			int iMod = 0; 
			if (pTargetCity->plot()->isHills())	
			{
				iMod += GC.getHILLS_EXTRA_DEFENSE();
			}
			if (GC.getGame().isOption(GAMEOPTION_MOUNTAINS) && pTargetCity->plot()->isPeak())
			{
				iMod += GC.getPEAK_EXTRA_DEFENSE();
...
//===NM=====Mountains Mod===0===== back to service
				if (plot()->isPeak())
				{
					iPlotMoveCost += GC.getPEAK_EXTRA_MOVEMENT();
				}
//===NM=====Mountains Mod===X===== back to service
				if (iPlotMoveCost > 1)
				{
					eMission = MISSION_ROUTE_TO;
				}
		...
		if ((bBuildRoute && (eRoute != NO_ROUTE)) || (bClearFeature && kBuildInfo.isFeatureRemove(eFeature)))
		{
			if (canBuild(pPlot, eBuild))
			{
				...
				if (bBuildRoute && (eRoute != NO_ROUTE))
				{
					...
					//===NM=====Mountains Mod===X=====keldath change 
					{
						int iMod = 0;
						if (iWorkersNeeded > 1) 
						{
							if (pPlot->isHills()) 
							{
								iMod += 2 * GC.getHILLS_EXTRA_MOVEMENT();
							}
							else if (GC.getGameINLINE().isOption(GAMEOPTION_MOUNTAINS) &&
									 pPlot->isPeak())
							{
								iMod += 2 * GC.getPEAK_EXTRA_DEFENSE();
							}
						}
					//===NM=====Mountains Mod===X=====
						iValue *= 2 + iWorkersNeeded + iMod; 
						iValue /= 3;
					}
