...
// < JImprovementLimit Mod Start >
						if (pMissionPlot->getTeam() != NO_TEAM && GC.getImprovementInfo(eImprovement).isNotInsideBorders() && GC.getImprovementInfo(eImprovement).isOutsideBorders())
						{
						    szBuffer.append(NEWLINE);
                            szBuffer.append(gDLL->getText("TXT_KEY_ACTION_REQUIRES_NO_CULTURE_BORDER"));
						}
//keldath added a null check
						else if (pHeadSelectedUnit != NULL && pMissionPlot->getTeam() != pHeadSelectedUnit->getTeam())
...
// < JImprovementLimit Mod Start >
                    ...
                    {
                        if (pMissionPlot->isImprovementInRange(eImprovement, GC.getImprovementInfo(eImprovement).getMakesInvalidRange(), true))
                        {
                            szBuffer.append(NEWLINE);
...
// < JCultureControl Mod Start >
                    if (GC.getImprovementInfo(eImprovement).isSpreadCultureControl() && GC.getGameINLINE().isOption(GAMEOPTION_CULTURE_CONTROL))
                    {
                        if (GC.getImprovementInfo(eImprovement).getCultureBorderRange() == 0)
                        {
                            szBuffer.append(NEWLINE);
                            szBuffer.append(gDLL->getText("TXT_KEY_ACTION_SPREADS_CULTURE_CONTROL_THIS_TILE"));
                        }
                        else if (GC.getImprovementInfo(eImprovement).getCultureBorderRange() == 1)
                        {
                            szBuffer.append(NEWLINE);
                            szBuffer.append(gDLL->getText("TXT_KEY_ACTION_SPREADS_CULTURE_CONTROL_RANGE_ONE", GC.getImprovementInfo(eImprovement).getCultureBorderRange()));
                        }
                        else if (GC.getImprovementInfo(eImprovement).getCultureBorderRange() > 1)
                        {
                            szBuffer.append(NEWLINE);
                            szBuffer.append(gDLL->getText("TXT_KEY_ACTION_SPREADS_CULTURE_CONTROL_RANGE", GC.getImprovementInfo(eImprovement).getCultureBorderRange()));
                        }
                    }
...
/*                                                                                              */
/************************************************************************************************/
	szTempBuffer.Format(L"%S", "KelDath kmod Platform");
	szBuffer.append(szTempBuffer);
/************************************************************************************************/
/* UNOFFICIAL_PATCH                       END                                                   */
/************************************************************************************************/
...
/************************************************************************************************/
	// Add string showing version number
	szTempBuffer.Format(NEWLINE SETCOLR L"%S" ENDCOLR, TEXT_COLOR("COLOR_POSITIVE_TEXT"), "Advc Kmod 1.46b");
	szBuffer.append(szTempBuffer);
	szBuffer.append(NEWLINE);
#ifdef LOG_AI
