...
void CvUnitAI::AI_attackCityMove()
{
	...
	if (bInCity && /* cdtw.9: */ getPlot().getTeam() == getTeam())
	{
		...
		{
		//qa-rb-1 -rangedattack-keldath my guess - dont leave city if you can bombard
			if (!AI_rangedStrikeK())
				return;
		}
		//if ((eAreaAIType == AREAAI_ASSAULT) || (eAreaAIType == AREAAI_ASSAULT_ASSIST))
...
//qa-rb-1 ranegdstrike - maybe add a check for bombardrate? or in the func it self? if any
	if (AI_rangedStrikeK()) 
	{
		return;
	}
