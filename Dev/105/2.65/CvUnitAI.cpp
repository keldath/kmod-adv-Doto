...
/*			if (bAggressive && (AI_rangedStrikeK(45)||
				AI_rangeAttack(airRange())))
			{
				return;
			}
			else*/ if (bAggressive && AI_anyAttack(3, 40))
				return;
		}
...
void CvUnitAI::AI_attackCityMove()
{
	...
	if (bInCity && /* cdtw.9: */ getPlot().getTeam() == getTeam())
	{
		...
		{
		//rangedattack-keldath my guess - dont leave city if you can bombard
			if (!AI_rangedStrikeK())
				return;
		}
		//if ((eAreaAIType == AREAAI_ASSAULT) || (eAreaAIType == AREAAI_ASSAULT_ASSIST))
...
//rangedattack-keldath
					 ...
					 AI_rangedStrikeK() ? true : AI_bombardCity())
...
//ranegdsteike - maybe add a check for bombardrate? or in the func it self? if any
	if (AI_rangedStrikeK()) 
	{
		return;
	}
...
//rangedattack-keldath -removed - i dont want ranged units to pillage.
			...
			/*	||canRangeStrikeAt(plot(), p.getX(), p.getY()) 
				||canRangeStrikeAtK(plot(), p.getX(), p.getY())*//*changed from canRangeStrike()*/)
			{
...
bool CvUnitAI::AI_anyAttack(int iRange, int iOddsThreshold, int iFlags, int iMinStack, bool bAllowCities, bool bFollow)
{
	...
	FAssert(canMove());
	//rangedattack-keldath
	if (AI_rangedStrikeK(iOddsThreshold),rangeStrike())
	{
			return true;
	}
...
//rangedattack-keldath
	if (getDomainType() != DOMAIN_AIR && (rangedStrike() < 1 || airRange() < 1))
	{
		if (kPlot.//isRoute()
...
int CvUnitAI::AI_rangedStrikeValueK(CvPlot const& kPlot, int iCurrentBest) const
{
	...
	// air strike (damage)
	//keldath - ranged attack have no need to check if the tile is movebale.
	/*if (canMoveInto(kPlot, true))
	{*/
		iAdjacentAttackers += GET_PLAYER(getOwner()).AI_adjacentPotentialAttackers(kPlot);
		//if (kPlot.isWater() || iPotentialAttackers > 0 || kPlot.isAdjacentTeam(getTeam()))
...
//	{
		if (kPlot.isCity())
		{
			...
			if(pCity->getDefenseModifier(true) > 0 /*|| AI_getGroup()->AI_compareStacks(*kPlot, true) > 50*/) // advc.004c
	...
	/* dont care for bonuses bombard for now*/
//	}
//removed bombard bool check
...
//intercept code was here - dont need
	return std::max(iBombValue, std::max(iCurrentBest, iStrikeValue));
}
...
//need to add code for city or peaks change
	for (SquareIter it(*this, rangedStrike(), false); it.hasNext(); ++it)
	{
			...
			if (!canRangeStrikeAtK(plot(), p.getX(), p.getY()) || !p.isVisibleEnemyUnit(this))
				continue;
			//bBombard will be checked in AI_rangedStrikeValueK
			int iValue = AI_rangedStrikeValueK(p, iBestValue/*, bBombardLoop*/);
