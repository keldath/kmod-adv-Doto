...
void CvGameTextMgr::setUnitHelp(CvWStringBuffer &szString, const CvUnit* pUnit,
	...
	if(!bOmitOwner && !pUnit->isUnowned()) // advc.061: Replacing the above
	{
		...
		szString.append(L", ");
//line below is new in advc 096 - not sure how it works with the below?
...
// BUG - Building Actual Effects - end
{
	...
	if(kBuilding.getRequiredInputBonusCount() > 0 && GC.getBuildingInfo(eBuilding).getBuildingOutputBonusCount() > 0 )
	{
		bool bFirst = true;
		...
		for (iI = 0; iI < GC.getNumBonusInfos(); iI++)
		{
			...
			if(kBuilding.getRequiredInputBonusValue((BonusTypes)iI) > 0)
			{
				...
				//swprintf(szTempBuffer, L"%d <link=literal>%s</link> (%c)", kBuilding.getRequiredInputBonusValue((BonusTypes)iI), GC.getBonusInfo((BonusTypes)iI).getDescription(), GC.getBonusInfo((BonusTypes)iI).getChar());
				szTempBuffer.Format(L"%d <link=literal>%s</link> (%c)", kBuilding.getRequiredInputBonusValue((BonusTypes)iI), GC.getBonusInfo((BonusTypes)iI).getDescription(), GC.getBonusInfo((BonusTypes)iI).getChar());
				szBuffer.append(szTempBuffer);
				bFirst = false;
			}				
		}
		...
		for (iI = 0; iI < GC.getNumBonusInfos(); iI++)
		{
			...
			if(kBuilding.getBuildingOutputBonusValues((BonusTypes)iI) > 0)
			{
				...
				//swprintf(szTempBuffer, L"%d <link=literal>%s</link> (%c)", kBuilding.getBuildingOutputBonusValues((BonusTypes)iI), GC.getBonusInfo((BonusTypes)iI).getDescription(), GC.getBonusInfo((BonusTypes)iI).getChar());
				szTempBuffer.Format(L"%d <link=literal>%s</link> (%c)", kBuilding.getBuildingOutputBonusValues((BonusTypes)iI), GC.getBonusInfo((BonusTypes)iI).getDescription(), GC.getBonusInfo((BonusTypes)iI).getChar());
				szBuffer.append(szTempBuffer);
				bFirst = false;
			}				
		}
...
/* Population Limit ModComp - Beginning */
	if (GC.getTechInfo(eTech).isNoPopulationLimit() && (!bPlayerContext || (GC.getGame().getActiveTeam() == NO_TEAM) || !(GET_TEAM(GC.getGame().getActiveTeam()).isNoPopulationLimit())))
	{
		if (bList)
		{
			szBuffer.append(NEWLINE);
...
/* not sure why commented out - keldath - the below is the  same but from bts
	// < Civic Infos Plus Start >
	int iBuildingCommerce = city.getBuildingCommerce(eCommerce) + city.getBuildingCommerceChange(eCommerceType);
	// < Civic Infos Plus End   >
...
//105 keldath from advc 097 - Bugfix: Wrong sign shown for anti-Barbarian modifiers
