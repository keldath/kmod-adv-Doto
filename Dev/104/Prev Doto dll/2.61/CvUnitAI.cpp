...
void CvUnitAI::AI_settleMove()
{
	...
	if (iAreaBestFoundValue == 0 && iOtherBestFoundValue == 0)
	{
		...
		{
//105 keldath deom advc 097 some changes - syntax mostly -AI tweak: Halt settler production when all sites taken
...
//105 keldath merge from f1rpo 097 = Amend some (not-so-recent) changes to defender selection
	...
	for (int iDX = -iSearchRange; iDX <= iSearchRange; iDX++)
	{
		for (int iDY = -(iSearchRange); iDY <= iSearchRange; iDY++)
		{
			...
			{
				int iPathTurns;
//vincentz ranged strike - keldath added 105 + Amend some (not-so-recent) changes to defender selection
				if (!atPlot(pLoopPlot) && (bFollow ? canMoveOrAttackInto(*pLoopPlot, bDeclareWar) :
					generatePath(pLoopPlot, iFlags, true, &iPathTurns, iRange))|| canRangeStrike())
				{
					if (iValue > iBestValue)
					{
						iBestValue = iValue;
...
//105 keldath merge from f1rpo 097 = Amend some (not-so-recent) changes to defender selection
		...
		ECT_ATTACK | MOVE_SINGLE_ATTACK : 0));
		return true;
	}
...
//keldath 105 change - i hope thats....ok
