...
/*changed to doto  - keldath */
		if(szThemePath.find("Doto") == CvString::npos)
			fontFactor = 14 / 12.0;
	}
	CvGame const& g = GC.getGameINLINE();
...
/************************************************************************************************/
	...
	if (ci.getStateReligionExtraHealth() != 0)
	{
		...
		{
			szHelpText.append(NEWLINE);
            szHelpText.append(gDLL->getText("TXT_KEY_CIVIC_STATE_RELIGION_EXTRA_HEALTH", abs(ci.getStateReligionExtraHealth()), ((ci.getStateReligionExtraHealth() > 0) ? gDLL->getSymbolID(HEALTHY_CHAR) : gDLL->getSymbolID(UNHEALTHY_CHAR)), GC.getReligionInfo(GET_PLAYER(GC.getGameINLINE().getActivePlayer()).getStateReligion()).getChar()));
		}
		else
		{
		    szHelpText.append(NEWLINE);
	...
	if (ci.getNonStateReligionExtraHealth() != 0)
	{
		...
		{
		    szHelpText.append(NEWLINE);
			szHelpText.append(gDLL->getText("TXT_KEY_CIVIC_NON_STATE_REL_EXTRA_HEALTH_WITH_STATE", abs(GC.getCivicInfo(eCivic).getNonStateReligionExtraHealth()), ((ci.getNonStateReligionExtraHealth() > 0) ? gDLL->getSymbolID(HEALTHY_CHAR) : gDLL->getSymbolID(UNHEALTHY_CHAR))));
		}
	}
...
/* not sure why commented out - keldath - the below is the  same but from bts
	// < Civic Infos Plus Start >
...
//civic info plus - added by f1 - help text - keldath
 	if(owner.getStateReligion() != NO_RELIGION && city.isHasReligion(owner.getStateReligion()))
       iCivicMod += city.getStateReligionCommerceRateModifier(eCommerceType);
    else iCivicMod += city.getNonStateReligionCommerceRateModifier(eCommerceType);
//civic info plus - added by f1 - help text - keldath	
	if (0 != iCivicMod)
	{
...
//civic info plus - added by f1 advc - help text - keldath
	if(owner.getStateReligion() != NO_RELIGION && city.isHasReligion(owner.getStateReligion()))
       iCivicMod += city.getStateReligionYieldRateModifier(eYieldType);
   	else iCivicMod += city.getNonStateReligionYieldRateModifier(eYieldType);
//civic info plus - added by f1 advc - help text - keldath
	if (0 != iCivicMod)
	{
		szBuffer.append(gDLL->getText("TXT_KEY_MISC_HELP_YIELD_CIVICS", iCivicMod, info.getChar()));
