...
int CvUnitAI::AI_sacrificeValue(const CvPlot* pPlot) const
{
	...
	else 
	{
		...
		//advc fix for negetive attack values - added by keldath
		//iValue /= (100 + cityDefenseModifier());
		int iCityDefenseModifier = cityDefenseModifier();
		if(iCityDefenseModifier < 0) {
   			iCityDefenseModifier = (2 * iCityDefenseModifier) / 5;
   			FAssert(iCityDefenseModifier >= -100);
		}
		iValue /= std::max(1, 100 + iCityDefenseModifier);
		iValue *= (100 + withdrawalProbability());
		iValue /= 100; // K-Mod
...
//original adv code - removed by keldath due to damage terrain
//	if(AI_heal(30, 1))
//		return;
/*****************************************************************************************************/
/**  Author: TheLadiesOgre                                                                          **/
	...
	for (int iBuild = 0; iBuild < GC.getNumBuildInfos(); iBuild++)
	{
		...
		if ((bBuildRoute && (eRoute != NO_ROUTE)) || (bClearFeature && kBuildInfo.isFeatureRemove(eFeature)))
		{
			if (canBuild(pPlot, eBuild))
			{
				...
				//===NM=====Mountain Mod===X=====
					{
						iValue *= 2 + iWorkersNeeded + (((pPlot->isHills() || pPlot->isPeak()) && (iWorkersNeeded > 1)) ? 2 * GC.getHILLS_EXTRA_MOVEMENT() : 0);
				//===NM=====Mountain Mod===0=====		
						iValue /= 3;
					}
