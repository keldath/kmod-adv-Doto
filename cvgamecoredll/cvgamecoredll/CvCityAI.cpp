...
/* removed section by kmod - left here to preserve pop limit code thats all*/
//by keldath
...
***/
	...
	if (iUnitSpending < iMaxUnitSpending + 25) // was + 10 (new unit spending metric)
	{
		...
		if( bBuildAssault )
		{
			...
			iTrainInvaderChance += (bAggressiveAI ? 15 : 0); // advc.019: was ?15:0//RECHANGED FOR DOTO
...
int CvCityAI::AI_experienceWeight()
{
	...
	return 2 * getProductionExperience() + getDomainFreeExperience(DOMAIN_LAND) + getDomainFreeExperience(DOMAIN_SEA)
			...
			- std::min(4, 2 * GET_PLAYER(getOwnerINLINE()).getCurrentEra());
	// K-Mod end
}
