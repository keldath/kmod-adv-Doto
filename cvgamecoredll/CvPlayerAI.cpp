	...
	CvArea const* pArea) const
{
	...
	if (!bValid)
	{
		...
		case UNITAI_COLLATERAL:
			...
			//ai seem to Due seIge a whole lot more.
...
//ranged attackers that have no capture city attribute that ui set
			//removed for now , keldath 1 1382021
		...
		FOR_EACH_UNITAI_IN(pLoopUnit, p)
		{
				...
				GET_TEAM(getTeam()).isAtWar(kUnit.getTeam())))
			{
				if (eDomainType == NO_DOMAIN || kUnit.getDomainType() == eDomainType)
				{
					...
					if (bCheckCanAttack)
					{
//doto ranged attack - could be optional to add refrence here 110
						if (!kUnit.canMove() ||
...
//doto ranged immunity 110 - do count ranged strike units 
						if(kUnit.combatLimit() >= 80 || kUnit.rangedStrike() > 0)
							(*piAttackerCount)++;
					} // </advc.139>
				}
			}
