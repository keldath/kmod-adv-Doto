...
class CvCivicsScreen:
	...
	def drawAllButtons(self):				
		...
		def govcivic(civicIdx, civicName_l, firstRow, panelLength, fY, line_seperator, civicoption, panel=PanelStyles.PANEL_STYLE_MAIN):
		...
		govCivics = self.m_allParentsCivics
		for l in xrange(len(govCivics)):
			if len(govCivics[l]) < 1 :
				continue
			for i in xrange(len(govCivics[l])):
				govcivic(i, govCivics[l][i], True, govPnelLength, fY, line_seperator - 15,  l, PanelStyles.PANEL_STYLE_BLUE50)
		...
		for j in self.m_allChildCivics.keys():
			if len(self.m_allChildCivics[j]) > 0:
				...
				for i in xrange(len(civicChild_l)):
					govcivic(i, civicChild_l[i], False , panelLength_l_val, fY, line_seperator, numCivicOtopns[j], PanelStyles.PANEL_STYLE_CITY_COLUMNL)
		...
		# goveRulesCivics = self.m_allChildCivics[1]
		...
		# legCivics = self.m_allChildCivics[2]
	...
	def select(self, iCivic):
		...
		# 	self.m_highLighterParent[iCivicOption] = iCivic
